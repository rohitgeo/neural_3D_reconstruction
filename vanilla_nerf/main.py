...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        if cam_idx == 0 and file_prefix == '':
            ...
            rend = render_points('out/1_4/sample_pts.png', sample_points.unsqueeze(0).cuda(), device='cuda')
...
def train(
    ...
    for epoch in t_range:
        ...
        if (epoch % 10) == 0:
            psnr = mse2psnr(loss)
            t_range.set_description(f'Epoch: {epoch:04d}, Loss: {loss:.06f}, PSNR: {psnr:.06f}')
            t_range.refresh()
...
mse2psnr = lambda x: -10.0 * np.log(x.detach().cpu().numpy() + TINY_NUMBER) / np.log(10.0)
